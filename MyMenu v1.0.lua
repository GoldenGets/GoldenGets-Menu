MyMenu = gui.get_tab("GoldenGets Menu")
...
MyMenu:add_imgui(function()
    if ImGui.Button("Switch CEO/President") then
		...
		if globals.get_int(1887305+playerIndex*609+10+430+1) == 0 then --1886967+playerIndex*609+10+429+1 = 0 ?CEO =1??????
			globals.set_int(1887305+playerIndex*609+10+430+1,1)
			gui.show_message("Switch as a President","Become a President of your own Motorcycle Club")
		else
			if globals.get_int(1887305+playerIndex*609+10+430+1) == 1 then
				globals.set_int(1887305+playerIndex*609+10+430+1,0)
	...
	--if gtaoversion:get_string() == "3323" then
	...
	if NETWORK.NETWORK_IS_SESSION_STARTED() then
		if ImGui.BeginTabBar("My Menu Tabs") then
...
script.register_looped("My Menu", function()
...
script.register_looped("My Cooldown", function (script)
	...
	if DispatchWorkCooldown then
		if globals.get_int(DWCl) ~= 0 then
			globals.set_int(DWCl, 0)
		end
...
------------------
MyBusiness = MyMenu:add_tab("Business")
------------------
		...
		if ImGui.BeginTabBar("My Business Tab") then
			...
			if ImGui.BeginTabItem("Resupply") then
				...
				if ImGui.Button("Collect Crates Amount##SpecialCargo") then
					locals.set_int(GCB, SCVIBl1, 1)
					locals.set_int(GCB, SCVIBl2, collectcrates)
					locals.set_int(GCB, SCVIBl3, 6)
					locals.set_int(GCB, SCVIBl4, 4)
				end
				if ImGui.IsItemHovered() then
				...
				if ImGui.Button("Warehouse Staff Sourced##SpecialCargo") then
					...
					stats.set_packed_stat_bool(32359, true)
					stats.set_packed_stat_bool(32360, true)
					stats.set_packed_stat_bool(32361, true)
					stats.set_packed_stat_bool(32362, true)
					stats.set_packed_stat_bool(32363, true)
				end
				ImGui.SameLine()
			...
			if ImGui.BeginTabItem("Selling") then
				...
				gui.show_message("Autoshipment","may show that the task failed, but you should get the money!")
					local integer = locals.get_int(GS, HCVISl1)  --Local_1934.f_1078 SMOT_HLPDROP2
					locals.set_int(GS, 1934 + 1035, integer) --1934 + 1035 = 2969
					gui.show_message("Autoshipment","may show that the task failed, but you should get the money!")
				end
				if ImGui.IsItemHovered() then
				...
				if ImGui.Button("Instant Sell##AcidLab") then
					if locals.get_int(FCALS) == true then -- https://www.unknowncheats.me/forum/3641612-post76.html
						if locals.get_int(FCALS, ALISCl1) >= 1 then
							locals.set_int(FCALS, ALISDl1 + 2, 10) -- Progress Delivery Mission
							locals.set_int(FCALS, ALISDl1 + 3, 10) -- Must Delivery Mission
							script:sleep(1000)
							locals.set_int(FCALS, ALISCl1, 2) -- Delivery Mission Passed
					--	else
...
script.register_looped("My Selling", function()
    ...
    if SpecialCargoShipmentType then
        if SCRIPT.GET_NUMBER_OF_THREADS_RUNNING_THE_SCRIPT_WITH_THIS_HASH(joaat(GCS)) ~= 0 then
            if locals.get_int(GCS, SCVMTl) ~= 12 then 
                gui.show_message("Special Cargo - Selling", "Sell with single Shipment/Tug, Boats")
                locals.set_int(GCS, SCVMTl, 12) -- gb_contraband_sell.c	iVar0 = MISC::GET_RANDOM_INT_IN_RANGE(0, 14); --Local_545.f_7 = iVar0;
            end
        end
    end
    ...
    if MCBusinessShipmentType0 then
        if SCRIPT.GET_NUMBER_OF_THREADS_RUNNING_THE_SCRIPT_WITH_THIS_HASH(joaat(GBCS)) ~= 0 then
            if locals.get_int(GBCS, MCBSTl) ~= 0 then
                ...
                locals.set_int(GBCS, MCBSTl, 0) -- gb_biker_contraband_sell.c	iVar0 = MISC::GET_RANDOM_INT_IN_RANGE(0, 13); --Local_704.f_17 = randomIntInRange;
            end    
        end
    end
...
MyMisc:add_imgui(function()
	if ImGui.BeginTabBar("My Misc Tab") then
		if ImGui.BeginTabItem("Misc") then
			...
			EnableVehicle = ImGui.Checkbox("Enable Removed Purchase Vehicles", EnableVehicle)
			if ImGui.IsItemHovered() then
